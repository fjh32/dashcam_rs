...
impl RecordingPipeline {
    ...
    /// Setup Source
    /// Setup Sinks
    /// Connect Source Tee to each Sink via GST Pads
    fn build_pipeline(&mut self) -> Result<()> {
        let source = self.source.as_mut()
            .context("No source set for pipeline")?;
        ...
        for sink in &self.sinks {
            let tee_src_pad = source_tee.request_pad_simple("src_%u")
    ...
    pub fn start_pipeline(&mut self) -> Result<()> {
        if self.pipeline_thread.is_none() {
            println!("Starting pipeline at {}", chrono::Local::now().format("%m-%d-%Y %H:%M:%S"));
    ...
    fn handle_bus_message(bus: &gst::Bus) -> bool {
        ...
        let msg = bus.timed_pop_filtered(
            ...
            &[gst::MessageType::Error, gst::MessageType::Element, gst::MessageType::Eos]
        );
